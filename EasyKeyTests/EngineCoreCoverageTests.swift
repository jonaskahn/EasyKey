...
final class EngineCoreCoverageTests: XCTestCase {
    ...
    // MARK: - SessionState.hasBase on nil optional
    ...
    // MARK: - SessionState.wTransformVowelIndex with consonant
    ...
    // MARK: - VietnameseEngine escape key
    ...
    // MARK: - VietnameseEngine left arrow key
    ...
    // MARK: - VietnameseEngine processBackspace: tone removal when mark exists on last atom
    ...
    // MARK: - VietnameseEngine tone repeat toggles off
    ...
    // MARK: - VietnameseEngine passThrough with tone on empty buffer (VNI)
    ...
    // MARK: - SmartSwitchStore: search with empty query returns all
    ...
    // MARK: - SmartSwitchStore: updateChoice when preference doesn't exist
    ...
    // MARK: - SmartSwitchStore: updateChoice when choice is identical
    ...
    // MARK: - SmartSwitchStore: updateChoice when different
    ...
    // MARK: - SmartSwitchStore: handleAppFocus with missing identity
    ...
    // MARK: - SmartSwitchStore: edit unknown preference
    ...
    // MARK: - SmartSwitchStore: reset unknown key
    ...
    // MARK: - SmartSwitchStore: handleAppFocus on existing preference (recorded path)
    ...
    // MARK: - VietnameseCharacters: isVowel with consonant

    func testIsVowel_WithConsonant_ReturnsFalse() {
        XCTAssertFalse(VietnameseCharacters.isVowel("t"))
    }

    // MARK: - VietnameseCharacters: baseVowel with non-vowel

    func testBaseVowel_WithNonVowel_ReturnsNil() {
        XCTAssertNil(VietnameseCharacters.baseVowel("t"))
    }

    // MARK: - VietnameseCharacters: mark(forVowel:) with unmarked vowel
    ...
    // MARK: - UnicodePrecomposedEncoding: vowel composition failure fallback
    ...
    // MARK: - TransformEngine: applyTransformW with non-w-transformable vowel
    ...
    // MARK: - TransformEngine: toneTargetIndex with multiple vowels and closing offglide
    ...
    // MARK: - TransformEngine: toneTargetIndex with diphthong ay
    ...
    // MARK: - TransformEngine: toneTargetIndex open syllable u-a
    ...
    // MARK: - TransformEngine: toneTargetIndex open syllable i-a
    ...
    // MARK: - TransformEngine: toneTargetIndex single vowel
    ...
    // MARK: - TransformEngine: toneTargetIndex no vowels
    ...
    // MARK: - TransformEngine: passThrough appends atom
    ...
    // MARK: - TransformEngine: passThrough preserves uppercase
    ...
    // MARK: - Converter: capitalizeSentences with trailing punctuation
    ...
    // MARK: - Converter: removeMarks then lowercase
    ...
    // MARK: - VietnameseEngine: quickTelex with w at start and empty buffer
    ...
    // MARK: - VietnameseEngine: quickWState transformedVowel with out-of-bounds guard
    ...
    // MARK: - ClipboardHistory: sort with pinned and unpinned entries at same time
    ...
    // MARK: - ClipboardHistory: insert duplicate unpinned entry
    ...
    // MARK: - ClipboardHistory: insert duplicate pinned entry inherits pin state
    ...
    // MARK: - ClipboardHistory: clearUnpinned
    ...
    // MARK: - ClipboardHistory: entries matching non-empty query
    ...
    // MARK: - ClipboardHistory: prune by age
    ...
    // MARK: - ClipboardHistory: setPinned reaches limit
    ...
    // MARK: - ClipboardHistory: setPinned not found
    ...
    // MARK: - ClipboardHistory: setPinned unpin
    ...
    // MARK: - MacroStore: save with fileURL exercises the full path
    ...
    // MARK: - MacroStore: matchCapitalization
    ...
    // MARK: - MacroStore: import legacy format

    func testPreviewLegacyImport_ConvertsArrowFormat() throws {
        let store = MacroStore()
        let legacy = "sig => m best\nbtw => by the way"
        let preview = try store.previewLegacyImport(legacy)
        XCTAssertEqual(preview.additions.count, 2)
        XCTAssertEqual(preview.additions.first?.trigger, "sig")
    }

    // MARK: - MacroStore: apply import with skip resolution
    ...
    // MARK: - MacroStore: changeActiveEncoding
    ...
    // MARK: - SettingsRepository: defaultFileURL without appSupport
    ...
    // MARK: - EasyKeySettings: Codable round-trip persists all fields
    ...
    // MARK: - VietnameseEncoding: VNIWindows roundtrip
    ...
    // MARK: - VietnameseEncoding: CP1258
    ...
    // MARK: - VietnameseEncoding: EncodingFactory
    ...
    // MARK: - Converter: TCVN3 ↔ Unicode roundtrip
    ...
    // MARK: - VietnameseCharacters: Vietnamese vowel construction
    ...
    // MARK: - VietnameseCharacters: D with stroke
    ...
    // MARK: - SessionState: initial values
    ...
    // MARK: - SmartSwitchStore: stableKey with only path
    ...
    // MARK: - SmartSwitchStore: search by key
    ...
    // MARK: - SmartSwitchStore: load from file with missing schema version
    ...
    // MARK: - SmartSwitchStore: clearAll with fileURL
    ...
    // MARK: - SmartSwitchStore: reset with fileURL
    ...
    // MARK: - SmartSwitchStore: updateChoice with missing identity
    ...
    // MARK: - SmartSwitchOptions defaults
    ...
    // MARK: - UnicodePrecomposedEncoding: D stroke
    ...
    // MARK: - UnicodePrecomposedEncoding: uppercase D stroke
    ...
    // MARK: - MacroStore: previewImport with duplicate in file
    ...
    // MARK: - MacroStore: previewImport invalid boolean
    ...
    // MARK: - Converter: convertClipboard with platform clipboard mock

    func testConvertClipboard_WithPlainText_ReplacesText() {
        let mockClipboard = MockConverterClipboard(plainText: "việt nam")
        Converter.convertClipboard(mockClipboard, configuration: ConverterConfiguration(transforms: [.removeMarks]))
        XCTAssertEqual(mockClipboard.plainText, "viet nam")
    }

    func testConvertClipboard_WithNilText_DoesNothing() {
        let mockClipboard = MockConverterClipboard(plainText: nil)
        Converter.convertClipboard(mockClipboard, configuration: ConverterConfiguration())
        XCTAssertNil(mockClipboard.plainText)
    }

    // MARK: - VietnameseEngine: quickWState none, no vowel index, buffer starts with consonant
    ...
    // MARK: - VietnameseEngine: wTransformVowelIndex with qu/gi onset
    ...
    // MARK: - VietnameseEngine: uppercaseFirstCharacter
    ...
    // MARK: - VietnameseEngine: uppercaseFirstCharacter when already uppercase
    ...
    // MARK: - VietnameseEngine: revertDStroke on non-stroke d
...
final class MockConverterClipboard: ConverterClipboard {
    var plainText: String?
    var html: Data?

    init(plainText: String? = nil, html: Data? = nil) {
