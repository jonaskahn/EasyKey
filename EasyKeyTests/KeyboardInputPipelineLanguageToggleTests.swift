...
final class KeyboardInputPipelineLanguageToggleTests: XCTestCase {
    func testToggleLanguage_DoesNotMutateSettingsLocally_EmitsToggleRequest() {
        ...
        let event = CGEvent(keyboardEventSource: nil, virtualKey: 49, keyDown: true)! // space or shortcut
        ...
        XCTAssertEqual(pipeline.currentSettings.input.language, .vietnamese, "Pipeline local settings should remain unchanged until canonical update flows back")
    }
}
