...
- Typing does not invoke translation and typed content is not uploaded.
...
Cloud translation is opt-in. EasyKey makes no translation request while you type, select text, edit source text, change languages, or choose a provider. Only an explicit **Translate** action sends submitted source text directly to the selected provider. Requests do not pass through an EasyKey server.
