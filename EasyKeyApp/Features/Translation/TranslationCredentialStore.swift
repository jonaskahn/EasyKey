...
struct KeychainTranslationCredentialStore: TranslationCredentialStoring {
    ...
    func save(_ apiKey: String, for provider: TranslationProviderID) throws {
        ...
        let updateStatus = access.update(baseQuery(for: provider), attributesToUpdate: updateAttributes)
        switch updateStatus {
        case errSecSuccess:
            return
        case errSecItemNotFound:
            break
        default:
            throw TranslationCredentialError.unexpectedStatus(updateStatus)
        }
