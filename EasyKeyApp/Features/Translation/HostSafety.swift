    ...
    private static func isPrivateOrLoopback(_ sa: UnsafePointer<sockaddr>) -> Bool {
        if sa.pointee.sa_family == sa_family_t(AF_INET) {
            return sa.withMemoryRebound(to: sockaddr_in.self, capacity: 1) { sin in
                ...
                // 127.0.0.0/8
                if (addr & 0xFF00_0000) == 0x7F00_0000 { return true }
                // 10.0.0.0/8
                if (addr & 0xFF00_0000) == 0x0A00_0000 { return true }
                // 172.16.0.0/12
                if (addr & 0xFFF0_0000) == 0xAC10_0000 { return true }
                // 192.168.0.0/16
                if (addr & 0xFFFF_0000) == 0xC0A8_0000 { return true }
                // 169.254.0.0/16 (link-local)
                if (addr & 0xFFFF_0000) == 0xA9FE_0000 { return true }
                // 100.64.0.0/10 (CGNAT)
                if (addr & 0xFFC0_0000) == 0x6440_0000 { return true }
                // 0.0.0.0/8 or 224.0.0.0/4
                if (addr & 0xFF00_0000) == 0x0000_0000 || (addr & 0xF000_0000) == 0xE000_0000 { return true }
                return false
            }
