...
| EasyKeyKit | `one.ifelse.easykeyKit` | `EasyKeyKit.version = "0.0.7"` | Keyboard service, event synthesis, compatibility rules |
...
| Type | Public surface |
|---|---|
| `EasyKeySettings` | `schemaVersion` (currently 8) + 9 option groups (`input`, `typing`, `macro`, `compatibility`, `smartSwitch`, `system`, `converter`, `clipboard`, `translation`); forgiving decoder |
